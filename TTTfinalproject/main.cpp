...
TicTacToeGame::TicTacToeGame() : row(0), column(0), turn('X'), draw(false), isPlayer2NPC(false) {
...
void TicTacToeGame::announce_winner(std::ofstream& file) {
 if (draw) {
...
void TicTacToeGame::playGame() {
 std::ofstream resultFile("gameResults.txt", std::ios::app); // Open the game results file
 char playAgain;
 do {
 initialize_game(); // Set up the game
 std::cout << "Tic Tac Toe Game\n";
 std::cout << "Do you want to play with an NPC? (y/n): "; // Ask if the player wants to play against an NPC
 std::cin >> playAgain;
 isPlayer2NPC = (playAgain == 'y' || playAgain == 'Y'); // Set the flag for playing against NPC
 display_board(); // Show the initial game board
 while (gameover()) { // Continue playing until the game is over
 if (!(isPlayer2NPC && turn == 'O')) { // If it's not the NPC's turn
 std::cout << "Player " << turn << ", enter a number: "; // Prompt the player for a move
 std::cin >> choice; // Read the player's choice
 ...
 if (choice < 0 || choice > 8 || board[row][column] == 'X' || board[row][column] == 'O') {
 std::cout << "Invalid Move\n"; // If the move is invalid, notify the player
 continue; // Skip the rest of the loop and prompt for a move again
 }
 board[row][column] = turn; // Place the player's symbol on the board
 } else {
 ...
 std::cout << "Would you like to play again? (y/n): "; // Ask if the players want to play again
 std::cin >> playAgain; // Get the player's decision
